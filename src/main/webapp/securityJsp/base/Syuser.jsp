...
<script type="text/javascript">
	...
	var showFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '查看用户信息info',
			url : sy.contextPath + '/securityJsp/base/SyuserForm.jsp?id=' + id  //SyUserInfo
		},'650','500');//宽度,高度
	};
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/syuser!deleteUser.sy', {//deleteUser  delete.sy
					id : id
				}, function() {
	...
	var createQRcode = function() {
		parent.$.messager.confirm('询问', '批量更新生成二维码？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/syuser!createQRcode.sy', {
				}, function() {
					grid.datagrid('reload');
				}, 'json');
			}
		...
		grid = $('#grid').datagrid({
			...
			frozenColumns : [ [  {
				...
				formatter : function(value, row) {
					...
					<%}%>
					<%if (securityUtil.havePermission("/base/syuser!deleteUser")) {%>
				...
				<td>
					<table>
						<tr>
							...
							<td><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-table_add',plain:true" onclick="createQRcode();">生成二维码</a></td>
						</tr>
					</table>
				</td>
			</tr>
