...
<script type="text/javascript">
	...
	var mainTabs;
	//模板修改 变更页面
	function changefun(newValue,loginname){
		var new_url;
		//根据
		if(newValue){
			if(newValue==1){
				new_url="static/share1";
			}else if(newValue==2){
				new_url="static/share2";
			}
			new_url=new_url +'.jsp?vcard='+loginname;
			document.getElementById("share1").src=new_url;  
		}
	}
	
	function changeRealQR(loginname){
			$.ajax({
                type: "post",
                newUrl:"",
                url: sy.contextPath + "/base/syuser!doNotNeedSessionAndSecurity_qr.sy?vcard="+userNAME ,
                success: function (data) {
                    var jsonobj=eval('('+data+')');//通过eval() 函数可以将JSON字符串转化为对象
                    if(jsonobj.qrmoban==1){
                    	newUrl="static/share1";
                    }else{
                    	newUrl="static/share2";
                	}
                    newUrl=newUrl +'.jsp?vcard='+userNAME;
                    document.getElementById("share1").src=newUrl;  
                }
            });
		}
...
<body id="mainLayout" class="easyui-layout">
	...
	<div data-options="region:'west',href:'',split:true, collapsible:false " title="导航" style="width: 500px; ">
		<iframe id="share1" src="<%=contextPath%>/static/share.htm"
             	 style="border: 0; width: 100%; height: 99%;" frameBorder="0">
          </iframe> 
	</div> 
	<div data-options="region:'center'" style="overflow: hidden;">
		<div id="mainTabs">
		  <div title="欢迎" data-options="iconCls:'ext-icon-heart'" id="data_Div">
		    <iframe src="<%=contextPath%>/securityJsp/base/SyWelcome.jsp"
